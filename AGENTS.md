...
|---|---|---|---|---|---|
| EASY | 첫날 | 10분 | **7.8분** (78%) | 6.2 | 100% |
| NORMAL | 5일차 | 15분 | **13.6분** (90%) | 6.3 | 100% |
| HARD | 10일차(상한) | 20분 | **12.8분** (64%) ✗ | 5.3 | 100% |

밀도는 세 난이도 모두 목표 안이다(오랫동안 3.0~3.4 로 하한을 밑돌던 문제는 아래 "교전선" 정리로 해결).
⚠️ **HARD 가 64% 로 짧다 — 미해결.** 아래 "긴 목표" 항목 참조.
...
**교전선 = 기체 위치 − 탄 사거리.** 다만 **절대선과 함께 써야 한다.**

`Player._bullet_range()` 는 `min(사거리 0.47, 상단 절대선 0.15 까지의 거리)` 를 쓴다.
세 가지를 다 겪고 나온 형태다:
1. **사거리만** → 기체가 올라가면 교전선이 스폰선까지 올라간다(원래 문제).
2. **기체를 하단에 묶기**(62%) → 문제는 해결됐지만 *"재미없다, 위로 더 올라갔으면"* 이라는
   지적을 받았다. 이동 자유를 뺏은 대가가 컸다.
3. **교전선만 고정** → 위로 갈수록 사거리가 짧아져 **위가 불리해진다.**
   실측 생존 96/87/76% → 57/51/46%. "위로 가고 싶다" 는 요구와 정반대다.
→ 둘 중 짧은 쪽을 쓰면 기체가 62% 아래일 때는 사거리가 그대로 걸려 밸런스가 유지되고,
   위로 올라가면 절대선이 걸려 적이 보이는 것이 보장된다.

⚠️ **기체 상한(`MIN_PLAY_Y_RATIO`)은 45% 다.** 33% 까지 올리면 안 된다 —
**완성 단어가 화면 33%** 에 나오므로(`WordReveal.WORD_ANCHOR`) 기체가 학습 대상을 스스로 가린다.
⚠️ 상한을 62% → 45% 로 풀면 그만큼 어려워진다(실측 생존 96/87/76% → 61/62/69%).
자유를 준 대가는 **적 연사**로 되돌렸다(0.72배). 자유 쪽이 재미의 본질이라 이 교환이 맞다.
