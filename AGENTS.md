...
|---|---|---|---|---|---|
| EASY | 첫날 | 10분 | **10.7분** (107%) | 4.1 | 100% |
| NORMAL | 5일차 | 15분 | **17.6분** (117%) | 3.3 ✗ | 100% |
| HARD | 20일차(상한) | 30분 | **32.7분** (109%) | 3.3 ✗ | 100% |

⚠️ **NORMAL·HARD 의 동시 적 수가 하한(3.5)에 못 미친다 — 미해결이다.**
예전 기록(6.6 / 5.2 / 4.4)은 낡은 값이었다. 적 변경 전후를 같은 조건으로 A/B 한 결과
둘 다 3.0 이라 원인은 적 변경이 아니고 그 전부터 있던 문제다.
적 10종으로 늘리며 3.0 → 3.3(EASY 3.4 → 4.1)까지 올라왔지만 아직 모자라다.
생존이 목표의 110~124% 인 것과 같은 뿌리다 — **적이 적어서 안 죽고, 그래서 오래 산다.**
`DifficultyDirector` 의 `spawn_interval` 배수를 낮추면 두 지표가 함께 목표로 간다.
⚠️ 이 표를 볼 때 **기록된 기준선을 믿지 말고 직접 재라.** 낡은 기준선 때문에 무관한
변경을 원인으로 오인할 뻔했다(A/B 로 걸렀다).
...
| 지표 | 값 |
|---|---|
| 전체 어휘 | **1008개** (67테마 — 기본 536 + 심화 472) |
| 단어 하나가 화면에 머무는 시간 | 1.91초 (온전히 보이는 시간 1.16초) |
| 모든 단어를 한 번씩 보는 데 | 완성 약 2000회 (`REVIEW_EVERY` 3 으로 1/3 이 복습) |
| 10분 세션 | 약 50~76개 완성 |
| 전체 어휘 1회 순회 | **약 4.4~6.7시간 (26~40세션)** — 하루 30분 상한이면 9~13일 |
...
⚠️ **심화는 테마마다 개수가 다르다**(4개 또는 8개). 4개로 박아 두지 마라 —
`get_pending_advanced()` 는 미수집 심화의 맨 앞 하나만 돌려주므로 개수와 무관하게 동작한다.
...
기존 블록과 **둘 다** 넣어서 `--write` 를 두 번 돌리면 테마가 중복됐다(실측 25개 → 44개,
19개가 두 벌). 지금은 `vocab_new` 에 있는 id 의 기존 블록을 버리고 새로 찍는다.

⚠️⚠️ **생성기는 확장 블록을 통째로 교체한다 — 블록 안 단어를 `existing` 에 넣으면 사라진다.**
`existing` 을 사전 전체에서 뽑으면 블록 안 단어가 "이미 있음" 으로 걸러져 새 블록에 안 담기고,
교체되는 순간 **사전에서 없어진다.** 실제로 겪었다: 1차 확장에서 사전 항목이 337 → 255 로 줄고
EGG·RICE·PIZZA 등이 통째로 빠졌다. 지금은 `existing` 을 **블록 바깥에서만** 뽑는다.
⚠️ **이 사고를 글자수·순서 검사가 통과시켰다.** `ThemeStages` 는 단어 **목록**만 갖고 있어서
사전이 비어도 순서는 멀쩡하다. 사전이 비면 이모지·한/영 설명·예문이 사라져 도감과 음성이 깨진다.
→ `VocabScaleCheck` 가 이제 **모든 테마 단어의 사전 항목·예문·이모지**를 확인한다.

⚠️ **어휘를 늘리면 단어만 늘지 않는다.** 총량에 딸린 것들이 같이 어긋난다:
```bash
godot --headless --path . scenes/VocabScaleCheck.tscn
```
"전부 수집" 훈장의 목표가 총량과 같은지 / 수집 해금 기체의 최고 기준이 총량의 60% 이상인지 /
**단어별 음성 파일이 빠지지 않았는지**를 확인한다.
- `Achievements` 의 `words300`·`theme25` 는 `"all": true` 로 **총량을 실시간으로** 쓴다.
  숫자를 박으면 단어를 추가한 순간 "전부 수집"이 전부가 아닌 지점에서 열린다.
- `StorePage` 의 "ALL n WORDS ARE FREE" 문구도 총량에서 가져온다.
- 같은 단어가 **두 테마에 들어가지 않았는지**도 본다(도감 수집이 한 번만 되므로
  두 테마의 완주 판정이 서로 어긋난다).
- 수집 해금 기체는 **최종 목표(1000개) 기준**으로 25/75/150/300/500/750/1000 이다(기체 17종).
  ⚠️ 기준을 총량에 비례해 움직이면 **이미 열린 기체가 다시 잠긴다** — 절대값으로 둔다.
  검사는 "최고 기준 < 총량의 60%" 일 때 FAIL 을 낸다(실제로 300 이었을 때 56% 로 걸렸다).
⚠️ 이 검사는 **씬으로** 돌린다. `--script` 모드에서는 `Achievements` 같은 `class_name`
스크립트 안의 오토로드 참조가 전역 식별자로 안 잡혀 컴파일에 실패한다(실제로 겪었다).
...
### 적 구성 — 7종이 실제로 7종으로 보여야 한다

[`EnemySpawner._pick_enemy_type`](scripts/EnemySpawner.gd) 는 **10종**을 내보낸다.
실측 개체 비중(첫날 300초): CHASER 19 / SHOOTER 18 / TANK 11 / DASHER 11 / SWARM 11 /
BOMBER 9 / SHIELDER 5 / PHANTOM 5 / SPLITTER 5 / TURRET 5 %.
문제는 **비율이 아니라 수명**이었다.
...
```
종류별 스폰 비율·수명 중앙값·사망 위치·**특수 행동 발동률**·적 탄 부하를 잰다.

⚠️ **"몇 마리 나오는가" 가 아니라 "얼마나 살아서 무엇을 했는가" 를 봐야 한다.**
첫 실측(첫날 300초)에서 드러난 것:

| 종류 | 수명중앙 | 특수발동 | 문제 |
|---|---|---|---|
| DASHER | 0.67s | 41% | 지그재그 한 주기가 0.785초라 **절반이 한 번도 못 꺾고 죽었다** |
| BOMBER | 1.07s | **0%** | 트리거 거리 150인데 탄 사거리가 768 — **한 번도 발동하지 못했다** |
| SHIELDER | 2.00s | 10% | 재생 주기 2.0초 = 수명 중앙값. 대부분 한 번도 회복 못 했다 |

즉 7종 중 셋이 **모양만 다른 CHASER** 였다. 소개 화면을 7종으로 늘려도 체감이 3종인 이유가 이것이다.

⚠️ **적은 화면 최상단에서 죽는다**(사망 위치 중앙 2~10%). 플레이어가 61% 에 있고
`Player.BULLET_RANGE_RATIO` 가 0.60 이라 탄이 화면 1% 까지 닿기 때문이다.
그래서 **"플레이어에게 접근해야 발동하는" 설계는 이 게임에서 성립하지 않는다.**
사거리를 줄이는 것은 답이 아니다 — 0.50 으로 줬을 때 밸런스가 무너진 기록이 있다(위 참조).
접근이 조건인 행동은 **격추 시점으로 옮기거나**(BOMBER) **수명 안에 들어오게 주기를 줄여라**(DASHER/SHIELDER).

⚠️ **특수 행동의 주기는 그 적의 수명보다 짧아야 한다.** 이 한 줄이 위 세 문제의 공통 원인이다.
새 행동을 넣을 때마다 `enemy_mix_check` 로 발동률을 확인할 것.

**나중에 넣은 3종은 이 제약을 전제로 설계했다:**
| 종류 | 정체성 | 제약을 어떻게 피했나 |
|---|---|---|
| SWARM 군체 | 다섯이 한 대형 | 스폰 순간에 이미 무리다 — 살아 있을 필요가 없다 |
| TURRET 포탑 | 멈춰 서서 쏜다 | 다가가지 않으므로 **수명이 체력으로만** 정해진다 |
| PHANTOM 환영 | 흐려지면 탄이 통과 | 위상 주기 0.35/0.5초 — 어떤 수명에도 들어온다 |

⚠️ **군체 확률은 다른 종류와 같게 두면 안 된다.** 한 번 뽑히면 `SWARM_COUNT`(5)마리가 나오므로
확률 0.05 로 뒀더니 **개체 비중이 22.4%** 가 되어 화면이 노란 마름모로 덮였다. 현재 상한 0.022.
⚠️ **포탑의 정지선은 적이 실제로 죽는 높이보다 위여야 한다.** 0.18 로 뒀더니 87% 가
도달 전에 죽어 발동률이 13% 였다. 0.08 로 낮춰 96% 가 됐다.
⚠️ **환영은 색만 바꾸면 안 된다.** 충돌을 끄지 않으면 흐린 동안 탄을 **먹어 버려서**
플레이어는 왜 안 죽는지 알 수 없다. `collision_layer = 0` 으로 통과시켜야 보인다.
⚠️ 확률 상한의 **합이 1을 넘으면 뒤쪽 종류가 영영 안 나온다**(누적 룰렛이다). 현재 합 0.89.

⚠️ 도구의 상수(`ZIGZAG_RATE`)는 `Enemy` 의 값과 맞춰야 한다. 게임만 바꾸면 옛 기준으로 재게 되어
수치가 통째로 무의미해진다(실제로 겪었다 — 고친 뒤에도 34% 로 나왔다).

⚠️ 적의 **탄 부하**를 함께 볼 것(도구가 발/초로 알려준다). 자폭병 격추 폭발을 넣자
4.0 → 4.8 발/초로 20% 늘었다. 탄이 늘면 밸런스가 움직이므로 반드시 `run_balance.sh` 로 재측정한다.
