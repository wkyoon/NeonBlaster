...
|---|---|---|---|---|---|
| EASY | 첫날 | 10분 | **9.6분** (96%) | 6.3 | 100% |
| NORMAL | 5일차 | 15분 | **12.9분** (86%) | 5.5 | 100% |
| HARD | 20일차(상한) | 30분 | **15.4분** (51%) ✗ | 5.1 | 100% |

✅ **동시 적 수가 처음으로 세 난이도 모두 목표(3.5~8.0) 안에 들어왔다.**
오랫동안 3.0~3.4 로 하한을 밑돌던 문제가 아래 "교전선" 정리로 해결됐다.

⚠️ **HARD 가 목표의 51% 에서 끝난다 — 미해결이다.** 구조적 원인이 있다:
치사율은 **정규화된 진행도**(경과/목표)로 오르는데 피해는 **절대 시간**으로 쌓인다.
목표 30분 판은 10분 판과 같은 곡선으로도 3배 오래 위험에 노출되므로 목숨 5개로는 못 버틴다.
`LETHAL_STRETCH`(목표가 길수록 치사율을 뒤로 미는 항)를 넣어 완화했지만 부족하다.
남은 선택지: 상한을 낮추기(`MAX_BONUS_MINUTES` 20 → 10, 목표 30분 → 20분) /
긴 목표에서 목숨·부활을 늘리기 / `LETHAL_STRETCH` 를 더 키우기.
⚠️ 분산이 크다 — 같은 설정에서 51%/60% 로 흔들린다. 한 번 재고 판단하지 말 것.
