...
- 하루 플레이할 때마다 목표가 **1분**씩 늘어난다(`MINUTE_PER_DAY`, 최대 +10분 = 목표 20분).
...
|---|---|---|---|---|---|
| EASY | 첫날 | 10분 | **11.4분** (114%) | 5.2 | 100% |
| NORMAL | 5일차 | 15분 | **13.6분** (91%) | 4.8 | 100% |
| HARD | 10일차(상한) | 20분 | **18.1분** (91%) | 4.8 | 100% |

✅ 세 난이도가 밀도·목표 시간·사망률을 모두 통과한다.
⚠️ HARD 는 같은 설정에서 70~91% 로 흔들린다. 이 폭은 5게임으로 가를 수 없다 —
판단이 필요하면 `./tools/balance_sweep.sh` 로 할 것.

✅ **세 난이도가 밀도·목표 시간·사망률을 모두 통과한다.**
오래 걸린 HARD(64%)도 아래 "교전선" 정리에서 **제한을 걷어내자** 함께 풀렸다 —
플레이어에게서 사거리를 뺏고 있었던 것이 원인이었다.

⚠️⚠️ **여기서부터는 단일 측정으로 노브를 만지지 마라.** 실제로 그 함정에 빠졌다:
적 체력을 0.85배로 내렸는데 EASY 78%→69% 로 **더 어려워졌고**(방향이 반대다),
`LETHAL_STRETCH` 를 키웠더니 NORMAL 90%→75% 가 됐다. 5게임으로는 10~15%p 차이를
분산과 구분할 수 없다. 다음 조정은 `./tools/balance_sweep.sh`(시드 x AI오차 격자)로 할 것.

⚠️ **긴 목표(HARD)는 목표 상한을 줄이는 것으로 못 고친다 — 자기 상쇄다.**
강도 곡선이 **목표에 정규화**돼 있어 목표를 줄이면 그만큼 절대 시간당 더 빨리 조여든다.
실측: 상한 20분 → 17분으로 줄였는데 생존 비율이 64% → 64% 로 그대로였다(절대 생존은 768s → 655s).
20 → 10 으로 줄일 때는 효과가 있었지만(51% → 71%) 그 뒤로는 듣지 않는다.
남은 선택지: 긴 목표에서 **목숨·부활을 늘리기**(피해가 절대 시간으로 쌓이므로 총 체력이 늘어야 한다) /
`LETHAL_STRETCH` 를 스윕으로 제대로 찾기.

⚠️ **목표 상한을 30분 → 20분으로 내렸다**(`MAX_BONUS_MINUTES` 20 → 10).
치사율은 **정규화된 진행도**(경과/목표)로 오르는데 피해는 **절대 시간**으로 쌓인다.
30분 목표는 10분 목표와 같은 곡선으로도 3배 오래 노출돼 목숨 5개로는 못 버텼다(실측 51%).
`LETHAL_STRETCH`(목표가 길수록 치사율을 뒤로 미는 항, 0.055)와 함께 76% 까지 맞췄다.
⚠️ 분산이 크다 — 같은 설정에서 51%/60% 로 흔들린 적이 있다. 한 번 재고 판단하지 말 것.
...
| 10분 세션 | 약 50~76개 완성 |
| 전체 어휘 1회 순회 | **약 4.4~6.7시간 (26~40세션)** — 하루 30분(누적 인정 상한)이면 9~13일 |
...
⚠️ **이 게임은 전체 플래시와 카메라 셰이크를 쓸 수 없다** — 중앙에 단어가 있고 그 가독성이
학습 시간을 직접 결정한다. 그래서 장르 표준 기법 중 **화면 중앙을 건드리지 않는 것만** 쓴다.

| 기법 | 어디에 | 왜 안전한가 |
|---|---|---|
| **히트스톱** `EffectsManager.hitstop()` | 적 처치 순간 0.045초, 배율 0.18 | 화면을 흔들지도 밝히지도 않는다 |
| **피격 플린치** `Enemy.KNOCKBACK` | 맞은 적의 **그림만** 튕긴다 | 몸을 안 밀어 밸런스에 영향 없음 |
| **아이템 자석** `PowerUp.MAGNET_RANGE` | 파워업이 105유닛 안에서 끌려온다 | 아이템을 쫓다 시선이 단어에서 떠나는 것을 막는다 |
| **위기 비네트** `EffectsManager.set_danger()` | 목숨 1개일 때 화면 **네 변만** 붉게 맥동 | 중앙을 덮지 않는다 |

```bash
godot --headless --path . scenes/EffectsCheck.tscn
```
비네트가 보이는 레이어에 있는지 / 단어 영역을 덮지 않는지 / 히트스톱이 실제로 시간을 늦추고
되돌리는지 확인한다.

⚠️⚠️ **연출은 조용히 안 보인다.** 위기 비네트를 캔버스 레이어 `-1` 에 두었더니 게임 배경 뒤에
깔려 아무것도 안 보였다. 월드 0 / HUD 10 / UI 20 사이인 **5** 가 맞다.
(PointLight2D 텍스처를 비워 글로우가 전부 죽어 있던 전례도 있다 — 아래 절 참조.)

⚠️ **`Engine.time_scale` 은 `EffectsManager` 한 곳에서만 만진다.** 전역이라 히트스톱과
TIME_SLOW 파워업이 서로를 덮어쓴다. 최종 배율 = `base_time_scale` x 히트스톱이고,
TIME_SLOW 는 `set_base_time_scale()` 로 들어온다.
⚠️ 히트스톱 잔여 시간은 **실시간으로** 줄여야 한다(`delta / Engine.time_scale`).
스케일된 delta 로 줄이면 느려진 만큼 정지가 늘어붙는다.
⚠️ **AI 플레이(벤치마크)에서는 히트스톱을 끈다.** 벤치마크가 `Engine.time_scale` 을 매 프레임
자기 값으로 덮어쓰므로 끼어들면 측정이 재현되지 않는다.

⚠️ **연출이 밸런스를 바꾼다 — 반드시 재측정할 것.** 두 번 걸렸다:
- 피격 넉백을 **몸에** 걸었더니 발당 7유닛 x 초당 8발로 적 이동을 30% 이상 상쇄해
  생존이 87/110/70% → 125/135/134% 로 무너졌다(너무 쉬워졌다). 그림만 흔들도록 바꿨다.
- 아이템 자석 범위를 220유닛으로 잡았더니 파워업 획득량이 급증해 129/131/122% 가 됐다.
  105유닛으로 줄이고 적 연사를 1.14배로 되돌려 맞췄다.
...
### 교전선 — "적이 화면에 나오기 전에 다 죽어 있다"

사용자가 두 번 지적한 문제다. 원인이 **세 개** 겹쳐 있었다.

⚠️⚠️ **요구사항을 정확히 읽어라.** 사용자가 말한 것은
*"적이 **보이지 않는 상태에서** 죽어 버리는 것이 싫다"* 였다.
"적이 화면 깊숙이 내려와야 한다" 가 **아니다.** 이 차이를 놓쳐서 세 번 헛돌았다:

| 시도 | 결과 |
|---|---|
| 탄이 화면 상단 절대선을 못 넘게 | 기체 바로 앞의 적도 못 맞힌다 → *"총알이 맞지 않아서 짜증나"* |
| 기체를 하단(62%)에 묶기 | *"재미없어졌어, 위로 더 올라갔으면"* |
| 교전선만 고정 | 위로 갈수록 사거리가 짧아져 위가 불리 (생존 96/87/76% → 57/51/46%) |

**답은 진입 보호 하나뿐이었다.** `Enemy.ENTRY_Y`(60px) 까지 내려오기 전에는 충돌을 꺼서
탄이 통과한다 — 몸이 화면에 다 들어온 뒤에야 죽는다. 그것으로 요구는 충족되고,
**사거리 제한도 기체 이동 제한도 필요 없다.**
제한을 전부 걷어내자 밸런스도 함께 맞았다(HARD 64% → 80%) — 플레이어에게서 사거리를
뺏고 있었던 것이 오래 걸린 HARD 문제의 원인이었다.

⚠️ `ENTRY_Y` 를 크게 잡지 마라. 무적 구간이 길어지면 탄이 통과하는 시간이 늘어
   그것대로 "총알이 맞지 않는다" 가 된다. 보이기 시작하는 최소 높이면 충분하다.
⚠️ **기체 상한(`MIN_PLAY_Y_RATIO`)은 0.15 다 — 화면의 85% 까지 올라간다.**
   62% → 45% → 15% 로 두 번 풀었고 그때마다 "묶지 마라" 는 지적이 나왔다.
   위로 파고드는 것 자체가 이 게임 재미의 큰 부분이다. 다시 조이지 말 것.
⚠️ 다만 **완성 단어가 화면 33%** 에 나온다(`WordReveal.WORD_ANCHOR`). 기체가 그 위로 가면
   단어를 가릴 수 있다 — 문제가 되면 기체를 반투명하게 하거나 단어를 옮길 것.
   **상한을 다시 조이는 것으로 풀지 마라.**
- 예전: 기체가 30~89% 를 자유롭게 오갔고 사거리가 0.60 이라, 기체가 61% 에 있으면
  탄이 화면 **1%**(스폰선)까지 닿았다. 실측 사망 위치 중앙 **2~10%**(25~128px) —
  적 반지름이 18~22px 이니 절반은 몸이 다 보이기도 전에 죽었다.
- 지금: `Player.MIN_PLAY_Y_RATIO`(0.62)로 기체 세로 이동을 하단에 묶고
  `BULLET_RANGE_RATIO` 를 0.47 로 줄였다. 교전선이 **15%(기체가 위) ~ 42%(기체가 아래)**
  로 고정돼 적이 최소 15% 까지는 반드시 내려온다.
  ⚠️ **두 상수는 한 쌍이다.** 한쪽만 바꾸면 이 보장이 깨진다.
  ⚠️ 이것이 장르 표준이다 — 스페이스 인베이더·갤러가는 기체를 최하단에 고정해
  교전 거리를 아예 상수로 만든다.

**진입 보호.** `Enemy._is_entering` 은 **선언만 되고 쓰이지 않는 죽은 변수**였다.
지금은 `ENTRY_Y`(46px)까지 내려올 때까지 `collision_layer = 0` 으로 두어 탄이 통과한다.
⚠️ 무적으로 두면 탄을 **먹어 버려** 왜 안 죽는지 알 수 없다 — 충돌을 꺼야 한다.
⚠️ 환영(PHANTOM)의 위상 전환이 이 보호를 뚫지 않게 `_set_phased` 에서 함께 확인한다.

**SPREAD 파워업이 판 내내 화력을 영구히 올렸다.** 레벨 2(2줄기) → 4(5줄기)면 **DPS 2.5배**다.
실측 **3레벨 도달 중앙값 6.1초**, 한 판 파워업 115개 — 시작 몇 초에 화력이 최대가 됐다.
AGENTS.md 가 보상 쪽에 적어 둔 원칙("무기 레벨을 정수로 올리지 마라")을 판 안의 파워업이
그대로 위반하고 있었다. 지금은 `SPREAD_DURATION`(8초) 일시 버프이고 기본 레벨은 2로 고정이다.

**결과 (첫날 300초 실측)**
| | 이전 | 지금 |
|---|---|---|
| 사망 위치 중앙 | 2~10% | **24%** (하위10% 11% / 상위10% 43%) |
| 몸이 다 들어오기 전 사망 | 약 50% | **5%** |
| 적 수명 중앙 | 0.6~2.3초 | **1.8~9.6초** |
| 동시 적 | 3.0~3.4 ✗ | **5.1~6.3** ✓ |

⚠️ **플레이어 탄속(`Player.BULLET_SPEED` 720)도 난이도 노브다.**
느릴수록 적이 죽기까지 시간이 늘어 화면에 오래 보인다(학습 시간이 늘어 좋은 방향).
대신 처치가 느려져 난이도가 오른다 — 실측: 800 → 640(0.8배) 에서 생존이 92/100/80 → 58/82/63 으로
무너졌고, 720(0.9배) 에서 72/75/88 이었다. 적 연사 0.88배로 되돌려 87/110/70 이 됐다.
⚠️ 예전에는 `Bullet.gd` 의 기본값(800)에 기대고 있었다. 적 탄은 자기 값을 명시하므로
   그 기본값은 사실상 플레이어 전용인데 코드에 드러나지 않았다 — 지금은 Player 에 상수로 둔다.

⚠️ **치사율 노브의 서열**(이 순서로 만질 것):
1. **적 연사**(`EnemySpawner._apply_stats` 의 `fire_rate`) — 가장 강하다. 0.7배로 줄여
   EASY 59%→96%, NORMAL 77%→81% 가 됐다.
2. **적 체력**(`enemy_hp`) — 선 아래로 얼마나 더 내려오는지를 정한다. HARD 33%→60%.
3. **탄속**(`bullet_speed`) — 회피 난이도.
4. **스폰 간격** — 밀도를 직접 건드리므로 마지막에.
⚠️ **적 속도를 낮추지 마라.** 느려진 적이 화면에 더 오래 남아 밀도와 탄이 함께 늘어
   오히려 어려워진다(실측 NORMAL 77%→67%, HARD 60%→49%). 두 번 겪었다.

⚠️ 남은 것: `SHIELDER` 재생 발동 10%, `TURRET` 정지 도달 24%. 교전이 좁은 띠에 몰려
집중 사격을 받으므로 재생·정지 전에 죽는다. 화력 서열을 더 만질 때 함께 볼 것.
...
⚠️⚠️ **`--script` 모드 도구에서 `Enemy.X` 처럼 클래스로 참조하지 마라.**
오토로드가 전역 식별자로 안 잡혀 `Enemy.gd` → `Bullet.gd` 까지 컴파일이 무너진다.
실측: 탄이 스크립트 없는 `Area2D` 가 되어 적이 한 마리도 안 죽고 **스폰 0** 으로 잡혔다
(측정이 조용히 거짓말을 한다). 값은 **인스턴스에서** 읽을 것(`e.get("ENTRY_Y")`).
