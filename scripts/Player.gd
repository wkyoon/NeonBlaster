...
## 탄 사거리(화면 높이 대비). 기체에서부터 재는 **최대** 거리다.
## ⚠️ 이 값이 곧 교전 띠의 두께다. 밸런스는 이 값(0.47)에 맞춰 조정돼 있다.
const BULLET_RANGE_RATIO := 0.47
## 탄이 넘어갈 수 없는 **화면 상단 절대선**(화면 높이 대비).
##
## ⚠️ 사거리만 두면 교전선이 기체를 따라 올라간다. 기체가 61% 에 있을 때 탄이 화면 1%
##    (스폰선)까지 닿아 적이 사망 위치 중앙 2~10% 에서 죽었다 — 몸이 다 보이기도 전이다
##    ("적이 화면에 나오기 전에 다 죽어 있어").
## ⚠️ 반대로 **교전선만 고정하면** 위로 갈수록 사거리가 짧아져 위가 불리해진다.
##    실측: 생존이 96/87/76% → 57/51/46% 로 무너졌다. 사용자는 "위로 더 올라가고 싶다" 고 했는데
##    올라가면 약해지는 구조는 그 요구와 정반대다.
## ⚠️ 그래서 **둘 중 짧은 쪽**을 쓴다: `min(사거리, 이 선까지의 거리)`.
##    기체가 62% 아래면 사거리(0.47)가 그대로 걸려 **밸런스가 안 바뀌고**,
##    그보다 위로 올라가면 이 선이 걸려 적이 화면에 보이는 것이 보장된다.
const ENGAGE_LINE_RATIO := 0.15
## 기체가 절대선보다 위로 올라갔을 때 남겨 주는 최소 사거리.
## 없으면 위로 붙는 순간 탄이 바로 사라져 고장으로 보인다.
const MIN_RANGE_RATIO := 0.18
## 기체가 올라갈 수 있는 **최상단**(화면 높이 대비).
## ⚠️ **완성 단어가 화면 33% 에 나온다**(`WordReveal.WORD_ANCHOR`). 기체가 거기까지 올라가면
##    이 게임의 학습 대상을 스스로 가린다. 그래서 여유를 두고 45% 로 잡았다.
##    62% 는 "재미없다, 위로 더 올라가고 싶다" 는 지적을 받았다.
const MIN_PLAY_Y_RATIO := 0.45
...
func _spawn_bullet(pos: Vector2, dir: Vector2) -> void:
	...
	bullet.max_travel = _bullet_range(pos, dir)
...
## 이 탄이 갈 수 있는 거리. **교전선까지의 거리**이고, 기체가 그 선보다 위면 최소 사거리다.
## ⚠️ 대각선 탄이 있으므로 세로 거리를 그대로 쓰면 안 된다 —
##    방향의 y 성분으로 나눠 "그 선에 닿기까지의 실제 이동 거리"를 구한다.
func _bullet_range(origin: Vector2, dir: Vector2) -> float:
	var by_range: float = _screen_size.y * BULLET_RANGE_RATIO
	if dir.y >= -0.01:
		return by_range
	var to_line: float = (origin.y - _screen_size.y * ENGAGE_LINE_RATIO) / -dir.y
	return minf(by_range, maxf(to_line, _screen_size.y * MIN_RANGE_RATIO))
