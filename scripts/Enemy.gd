...
## 이 높이까지 내려와야 맞기 시작한다. 도형 반지름(최대 22)보다 넉넉히 아래라
## **몸이 화면에 다 들어온 뒤**에야 죽는다 — 이것이 "안 보이는 상태에서 죽지 마라" 의 보장이다.
## ⚠️ 이 값을 크게 잡지 마라. 무적 구간이 길어지면 그만큼 탄이 통과하는 시간이 늘어
##    "총알이 맞지 않는다" 로 느껴진다. 보이기 시작하는 최소 높이면 충분하다.
const ENTRY_Y := 60.0
