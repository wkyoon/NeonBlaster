...
## 상한. 목표 시간은 `BASE_TARGET`(10분) + 이 분수까지 늘어난다.
## ⚠️ **20 이었다(목표 30분). 목숨 5개로 30분을 버틸 수 없어 10 으로 내렸다.**
##    치사율은 정규화된 진행도로 오르는데 피해는 **절대 시간**으로 쌓인다 —
##    30분 목표는 10분 목표와 같은 곡선으로도 3배 오래 노출된다(실측 생존 51%).
##    이 게임은 틈새 시간용이라 한 판 20분도 충분히 길다.
const MAX_BONUS_MINUTES := 10
